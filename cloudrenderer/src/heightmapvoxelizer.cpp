...
{
    auto result = std::make_unique<eng::rndr::Texture3d>(nullptr, eng::rndr::TextureInfo{GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR, GL_RGBA, 4, GL_RGBA8, GL_UNSIGNED_BYTE}, (bbox.second.x-bbox.first.x)/voxelScale, (bbox.second.x-bbox.first.x)/voxelScale, (bbox.second.x-bbox.first.x)/voxelScale);
    ...
    glBindImageTexture(1, result->id(), 0, false, 0, GL_READ_WRITE, GL_RGBA8);
