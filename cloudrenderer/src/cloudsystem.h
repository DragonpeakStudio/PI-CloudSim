...
class CloudSystem : public eng::Object
{
    public:
        ...
        static std::shared_ptr<CloudSystem> create(std::pair<glm::vec3, glm::vec3> bbox, std::weak_ptr<OutdoorLighting> lighting, float voxelScale)
        {
            ...
            return std::shared_ptr<CloudSystem>(new CloudSystem(std::make_unique<Renderer>(bbox), std::make_unique<Simulator>(bbox, voxelScale), bbox, lighting));
        }
    ...
    private:
        CloudSystem(std::unique_ptr<CloudRenderer> renderer, std::unique_ptr<CloudSimulator> simulator, std::pair<glm::vec3, glm::vec3> bbox, std::weak_ptr<OutdoorLighting> lighting);
